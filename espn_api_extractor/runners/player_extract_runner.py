...
class PlayerExtractRunner:
    ...
    async def run(self) -> Union[List[Player], List[PlayerModel]]:
        ...
        try:
            # Step 1: Execute extraction via controller
            results: Dict[str, Any] = await self.controller.execute()
            ...
            # Step 2: Save extracted data to JSON (for next ETL stage)
            self._save_extraction_results(pitchers, batters, failures)

            # Step 3: Return players (convert to models if requested)
