...
class TestProjectionsExtraction:
    ...
    def test_get_player_cards_builds_correct_filters(self, fantasy_requests):
        ...
        with patch.object(fantasy_requests, '_get') as mock_get:
            ...
            # Check params
            assert call_args[1]['params'] == {"view": "kona_playercard", "scoringPeriodId": 0}
            ...
            assert filter_data['players']['filterStatsForTopScoringPeriodIds']['value'] == 1
            
            # Verify additional values include projections and seasonal stats
            additional_values = filter_data['players']['filterStatsForTopScoringPeriodIds']['additionalValue']
            ...
            assert "012025" in additional_values  # preseason stats
    ...
    def test_extract_projections_from_fixture(self, projections_fixture_data):
        ...
        preseason_stats = None
        regular_season_stats = None
        
        for stat in stats:
            if stat["id"] == "102025":
                projection_stats = stat
            elif stat["id"] == "012025":
                preseason_stats = stat
            elif stat["id"] == "022025":
                regular_season_stats = stat
        
        assert projection_stats is not None, "Projection stats (102025) should exist"
        assert preseason_stats is not None, "Preseason stats (012025) should exist"
        assert regular_season_stats is not None, "Regular season stats (022025) should exist"
    ...
    def test_extract_preseason_regular_and_previous_season_stats(self, projections_fixture_data):
        """Test extracting preseason, regular season, and previous season stats from fixture"""
        players = projections_fixture_data["players"]
        ohtani = players[1]  # Second player is Shohei Ohtani
        
        assert ohtani["id"] == 39832
        stats = ohtani["player"]["stats"]
        
        # Find preseason, regular season, and previous season stats
        preseason_stats = None
        regular_season_stats = None
        ...
        assert preseason_stats is not None
        assert regular_season_stats is not None
        assert previous_season_stats is not None
        
        # Test preseason stats
        pre_stats = preseason_stats["stats"]
        assert "0" in pre_stats  # AB
        assert pre_stats["0"] == 31.0  # Preseason AB
        
        # Test regular season stats  
        reg_stats = regular_season_stats["stats"]
        assert "0" in reg_stats  # AB
        assert reg_stats["0"] == 58.0  # Regular season AB
    ...
    def test_multiple_players_data_structure(self, projections_fixture_data):
        ...
        for player in players:
            ...
            # Should have at least projections, preseason, and regular season
